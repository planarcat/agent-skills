...
1. **按 id 批量查状态**（任务行来源）：`get_stories_or_tasks --id "id1,id2,…"`，一次多个 id 也支持；状态按 §7.1 中文语义转（**`planning`＝评审中**，不是"待开发"）。
2. **找出"没有预计完成时间"的在途工单**（周报①「计划」列不许留「—」，评审点名）：按项目拉在途单时把 `due` 为空或 `0` 的挑出来，列成待补清单 → 补 due + 优先级**回写 TAPD**（补之前把日期方案给用户确认，别自己拍）。
3. **只有标题没有编号 → 反查 id**：按 `name=%关键词%` 搜；搜不到就如实说找不到（**不许硬编 ID**，ID 错＝虚报）。截图里读出的编号也要用这条核一遍。

补完 TAPD 再写报告——「状态/计划即时更新是交报表的前置动作」。
