...
## 开发中不为日报提问（2026-09-21 用户口径）

**开发按原节奏走，不要为了写日报额外插入"要怎样怎样"。**

- 该问的开发问题（技术方案、需求边界、实现选择、风险）**照常问**——日报不该改变开发节奏。
- **不要为了填日报提问**（"这条和计划比一致吗""谁跟你一起做的""卡点期望时间""明天做什么"）：这些自己**记初判 + 标"待确认"**，攒到出日报时一次性问。
- **落卡零打扰**：素材卡是 agent 自己的事，不让用户配合格式、不让用户确认格式、不因此打断用户。
- 唯一例外（本来也是开发问题）：继续做会返工/做错方向 → 当场问，但必须**给出默认做法**，对方只回"行/不行"。
