...
1. **存放路径**：素材卡 `.workbuddy/reports/cards/YYYY-MM-DD.md`、日报 `日报_YYYY-MM-DD.md`。
   换机器/换工具时，把技能里出现的这些路径改成目标环境的实际路径。
2. **人名与项目名**：技能里写死了何成标的项目名（灵创、虾皮 POD 等）和对接人姓名。
   给别人用要先替换，否则会写出别人的名字。
3. **目录名含中文**：`日报、周报、月报/` 这个文件夹名带顿号，某些工具的命令行会踩坑，
   必要时改成 `reports/`。
