...
**下一步（日报硬规则）**：开工前先做一次**需求拆解**（`requirement-breakdown`）——日报 v3 要求需求首次进「开发中」前必须写过一次需求理解；没装那个技能也要自己写清边界 / 歧义 / 待确认点再动手。拆解文档落在**你开工的那个目录**（worktree 场景 = worktree 里的 `Docs/需求拆解/`，跟分支走），是否提交进仓库由你决定。
