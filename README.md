...
按改动规模与类型**自动分级**：**L0** 免审（纯文案 / 锁文件，跑 lint 自查）→ **L1** 快审（≤2 文件 ≤50 行，单代理）→ **L2** 定向审（按类型路由 2–3 个维度）→ **L3** 全量（大改动 / 高风险路径 / 提 MR 前，六个维度子代理）。「全面」强制 L3，「快速」强制 L1。只审不代改。六个维度的审查标准内置在本技能 `references/` 下，派遣子代理或降级自查时直接读取——单技能交付，无需单独安装维度。
