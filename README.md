...
| 🔍 **代码审查** | `full-code-review` | 智能分级审查（L0–L3）：小改快审、按类型定向、提 MR 前六维全量；只审不代改（6 维标准内置） |
...
├── full-code-review/             # 代码审查：智能分级 L0–L3（6 维标准内置）
...
├── report-pipeline/              # 日报：共享日报采集与共写
├── report-draft-filter/          # 日报：工作小结去噪分拣
├── report-writer/                # 日报：收口（内附规范源文档）
...
#### 11. 日报编写（共享日报 · 2026-09-22 口径）

三个技能各管一段，按 **共写 → 收口** 配合：

| 技能 | 管什么 | 触发示例 | 落盘 |
|:---|:---|:---|:---|
| `report-pipeline` | 采集与共写：阶段完成即更新共享日报里自己会话的块 | 「记一下」「整理今天的工作」 | `<工作区根>/Reports/daily_work/YYYY-MM-DD.md` |
| `report-draft-filter` | 分拣：口语化小结去噪、归类成日报条目 | 「这些哪些该写」「把这段小结整理成日报」 | 同一份文件 |
| `report-writer` | 收口：补今日总览、核对必填、红线自检（内附规范原文） | 「出日报」「检查这份汇报合不合规」 | 同一份文件（不另立成稿） |

**核心规则（2026-09-22 用户口径）**：

- 每个工作区**每日只维护一份日报**：`Reports/daily_work/YYYY-MM-DD.md`；不建素材卡、不另存成稿，所有 agent 都在这份文件上更新。
- **以工作分支为维度**：一个分支一节；同一分支的多个会话**共同维护该分支的「工作总结」**——A 开发登录模块、D 在 A 基础上继续，两人共写登录模块节；B 修 bug、C 加强密码校验，各自维护自己的节。
- **只更新自己的会话记录**：不重排、不删改他人内容；写入前先重读最新文件，只做局部修改。
- **每个工作节点带时间**：工作总结、会话记录、卡点 / 异常 / 明日 / 复盘的每条都带 `[HH:MM]` 时间要素——A、D 共写同一份总结时按时间排序、归属清楚，不会写乱。
- 内容质量仍按 v3 规范：TAPD ID 不许编、四要素、执行差异、协同人、卡点 6 项、敷衍话术禁令；提交时限次日 09:30 前。
- 需要正式 v3 五栏提交文本（评分器解析用）时，由 `report-writer` 从共享日报一次性生成并直接交付，不在工作区另存第二份。

**日常怎么用（两拍）**：

```text
① 做完一件事  → 说「记一下」，或直接把工作小结 / 语音转写稿丢过来
     → 更新共享日报里自己会话的块（分支小节：会话记录 + 工作总结相关句）

② 当天收工前  → 说「出日报」
     → report-writer 在共享日报上收口：补今日总览、核对必填、跑红线自检
```

- 不用刻意说「记一下」——发一段工作小结、贴个语音转写稿，技能都会按采集规则处理。
- 精确数字（测试数、耗时、前后对比）当天不记就容易失真，这是两拍节奏存在的唯一理由。
- 周报按需：落 `Reports/weekly/`；采集时额外记计划/实际完成日与方案链接版本号。
- 所有文件都在**工作区**里，不在技能目录下；落点在 git 仓库时，自行决定是否把 `Reports/` 加进 `.gitignore`。
- 项目名不写死，取自当天素材；唯一写死的是报告作者本人姓名（何成标）。
